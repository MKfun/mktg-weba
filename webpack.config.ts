...
const DEFAULT_APP_TITLE = `MKTG${APP_ENV !== 'production' ? ' Beta' : ''}`;
...
const {
  BASE_URL,
  ELECTRON_HOST_URL = 'https://mktg-a-host',
  APP_TITLE = DEFAULT_APP_TITLE,
} = process.env;

const CSP = `
  default-src 'self';
  connect-src 'self' wss://*.mktg.pooziqo.xyz blob: http: https: ${APP_ENV === 'development' ? 'wss:' : ''};
